PROFILE_DIR=${PROFILE_DIR:-/mnt/post-training-ppo/projects/z3n/profile_z1_llama3_8b}
mkdir -p ${PROFILE_DIR}
...
MODEL="/mnt/post-training-ppo/models/meta-llama/Meta-Llama-3-8B-Instruct"
...
for BATCH_SIZE in ${BATCH_SIZE_OPTS[@]}; do
    for SEQ_LENGTH in ${SEQ_LENGTH_OPTS[@]}; do
        ARGS="--model ${MODEL} --batch-size ${BATCH_SIZE} --seq-length ${SEQ_LENGTH} --zero-stage 1 ${ACC_OPTS} ${AC_OPTS} ${PROFILE_OPTS}"
